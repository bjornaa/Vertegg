function A = lwendrof(A0, K, W, nstep, dt, P, alpha)
...
elseif (nargin == 7)
  ...
  error('lwendrof must have 5 or 7 arguments');
end
...
Bm = 0.5*(C(II)-C(II).*C(II)) - S(II);  % Coeff. to A(Im)
B  = 0.5*(C(II)+C(II).*C(II)) + S(II);  % Coeff. to A(II)
