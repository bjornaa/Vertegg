function A = minlim(A0, K, W, nstep, dt, P, alpha)
...
for t = 1:nstep
  ...
  % 
  % Not checking boundary grid cells
  IJ = 1 + find((A1(2:Ncell-1) < A1(1:Ncell-2)) & ...
                (A1(2:Ncell-1) < A1(3:Ncell)));
  F(IJ)   = BUSm(IJ-1) .* A(IJ-1) + BUS(IJ-1) .* A(IJ);
  F(IJ+1) = BUSm(IJ)   .* A(IJ)   + BUS(IJ)   .* A(IJ+1);
  if (length(IJ) > 0)  % use limited flux
